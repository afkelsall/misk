...
//    callService(0).call()
    ...
    serviceLogsOrdered.groupBy { it.mdcPropertyMap[NestedTaggedLoggersBothSucceed.EXECUTION_IDENTIFIER] }
      .forEach { (_, logs) ->
        ...
        assertThat(logs[1].mdcPropertyMap).containsEntry("testTag", "SpecialTagValue123")
  ...
  class NestedTaggedLoggersBothSucceed @Inject constructor() : WebAction {
    ...
    fun call(@RequestHeaders headers: Headers): String {
      val result = logger
        ...
        .asContext {
          ...
          Thread {
            result = AnotherClass().functionWithNestedTaggedLogger(logger)
    ...
    class AnotherClass() {
      fun functionWithNestedTaggedLogger(parentTags: TaggedLogger<*,*>): String {
        return logger
          .inheritContextFrom(parentTags)
